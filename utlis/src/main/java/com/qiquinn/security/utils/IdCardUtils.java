...
public class IdCardUtils
{
    ...
    public static boolean checkIdCardDate(String idCardDate)
    {
        ...
        String formatDate = dateFormat.format(idCardDate);
        return DateUtils.isVerdifyDate(formatDate);
    }
    ...
    {
        Pattern pattern = Pattern.compile("^[1-9]\\\\d{5}[1-9]\\\\d{3}((0\\\\d)|(1[0-2]))(([0|1|2]\\\\d)|3[0-1])\\\\d{3}([\\\\d|x|X]{1})$");
    ...
    public static boolean checkPowerCode(String idCard)
    {
        ...
        {
            idcardArray[i] = Integer.valueOf( tmp[i] );
        }
